...
if (mysqli_query($conn, $sql)) {
    echo "<p>Conteneurs attribués. Ports : <strong>" . implode(", ", $ports_to_assign) . "</strong></p>";
}
...
while ($row = mysqli_fetch_assoc($containers_res)) {
    ...
    $command = "docker exec -u root $container bash -c \"id $user_esc >/dev/null 2>&1 || (useradd -m -s /bin/bash $user_esc && echo '$user_esc:$pass_esc' | chpasswd)\"";
    exec($command, $output, $return_var);

    if ($return_var === 0) {
        echo "<p><strong>Accès SSH créé :</strong><br>";
        echo "Identifiant : $user_name<br>";
        echo "Mot de passe : $user_password</p>";
    }
}
